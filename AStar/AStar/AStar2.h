...
// TODO
// Увеличить масимально допустимое разрешение карты
// Добавить карту отслеживания резких перепадов( для обхода ям или вертикальных препатствий)
// Возможность добавить опорный ракурс(клик по точке пути -> ренедер три д модели с этого ракурса)
// Поиск ближайшей секции
// Рендер с указанного ракурса
// Добавить интерфейс для работы с картой(Обсудить с Даниилом / Павлом Гиригорьевичем / )
// По возможности добавить трансляцию результатов одометрии инерциалки и оптики
// Движение по карте с корректировкой по ракурсу
class AStar2
{
private:
