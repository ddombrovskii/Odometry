...
def bno_request(bno_handle, request: int):
    message = request.to_bytes(1, 'big')
    write_package(bno_handle, message)
    time.sleep(0.01)
    response = read_package(bno_handle)
    if response[0] != 1:
        return None
    response = response[1:len(response) - (len(response) - 1) % 8]
    return struct.unpack(f'<{len(response) // 8}d', response)


def bno_test():
    ports = list_ports.comports()
    [print(p) for p in ports]
    with serial.Serial('COM7', baudrate=115200, timeout=10, bytesize=8, stopbits=serial.STOPBITS_ONE) as bno_port:
        for i in range(10):
            response = bno_request(bno_port, 1)
            print(response)
            time.sleep(1)
    # print(response)
...
def self_response():
    ports = list_ports.comports()
    [print(p) for p in ports]
    port = serial.Serial('COM5', baudrate=115200, timeout=1, bytesize=8, stopbits=serial.STOPBITS_ONE)
    data_int = tuple((i for i in range(10)))
    data_flt = tuple((i * 1.5 for i in range(10)))
    write_int_package(port, data_int)
    time.sleep(0.01)
    data = read_int_package(port)
    print(data)
    write_float_package(port, data_flt)
    time.sleep(0.01)
    data = read_float_package(port)
    print(data)
    port.close()
    # data = s.read(4 * 5 )
...
class AccelerometerBNO055:
    ...
    def __init__(self):
        ...
        self._bno_port = serial.Serial(target_port, baudrate=115200,
    ...
    def __str__(self):
        ...
        # data.append(f"\t{TIME:20}: {str(self.current_time if self.current_time>0 else 0.0)}")

        data.append(f"\t{DTIME:20}: {str(self.delta_t)}")

        if _is_bit_set(self._read_config, AccelerometerBNO055.ACCELERATION_BIT):
            data.append(f"\t{ACCELERATION:20}: {str(self.acceleration)}")

        if _is_bit_set(self._read_config, AccelerometerBNO055.OMEGA_BIT):
            data.append(f"\t{ANGLES_VELOCITY:20}: {str(self.omega)}")

        if _is_bit_set(self._read_config, AccelerometerBNO055.ANGLES_BIT):
            data.append(f"\t{ANGLES:20}: {str(self.angles)}")

        if _is_bit_set(self._read_config, AccelerometerBNO055.QUATERNION_BIT):
            data.append(f"\t{QUATERNION:20}: {str(self.quaternion)}")

        if _is_bit_set(self._read_config, AccelerometerBNO055.MAGNETOMETER_BIT):
            data.append(f"\t{MAGNETOMETER:20}: {str(self.magnetometer)}")

        if _is_bit_set(self._read_config, AccelerometerBNO055.ACCELERATION_LINEAR_BIT):
            data.append(f"\t{ACCELERATION_LIN:20}: {str(self.acceleration_linear)}")
        sep = ',\n'
    ...
    def record(self, file_path: str, time_out: float = 0.1, record_time: float = 5.0):
