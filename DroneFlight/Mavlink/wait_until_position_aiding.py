...
def _get_enum_value_by_name(enum_dict, name):
    """
...
def wait_until_position_aiding(mav_connection, timeout=120):
    ...
    while True:
        if ekf_pos_aiding(mav_connection, flags, estimator_status_msg) or time.perf_counter() - time_start > timeout:
            break
        print(f"Waiting for position aiding: {time.perf_counter() - time_start} seconds elapsed")
...
def ekf_pos_aiding(mav_connection, flags, estimator_status_msg="ESTIMATOR_STATUS"):
    ...
    for flag in flags:
        flag_val = _get_enum_value_by_name(mavutil.mavlink.enums["EKF_STATUS_FLAGS"], flag)
