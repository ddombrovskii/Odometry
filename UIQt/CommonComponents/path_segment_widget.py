...
class PathSegmentWidget(QWidget):
    ...
    def _init_layouts(self):
        ...
        common_layout.setContentsMargins(0, 10, 0, 0)
        ...
        group.setTitle("Границы сегмента")
        layout = QHBoxLayout()
        layout.setContentsMargins(0, 10, 0, 0)
        # group.setStyleSheet("border:1px solid rgb(41, 41, 41); ")
        ...
        path_post_process_layout.setContentsMargins(0, 10, 0, 0)
        # path_post_process_group.setStyleSheet("border:1px solid rgb(41, 41, 41); ")

        container, self._decimate_drop_down = init_drop_down(path_post_process_group, label_str="Децимирование",
                                                             values=
                                                             ["нет", "каждый второй",
                                                              "каждый четвёртый", "каждый восьмой"])
        ...
        container, self._smoothing_drop_down = init_drop_down(path_post_process_group, label_str="Сглаживание",
                                                              values=
                                                              ["нет", "4 точки", "8 точки", "16 точки", "32 точки"])
        path_post_process_layout.addWidget(container)
        ...
        path_movement_layout.setContentsMargins(0, 10, 0, 0)
        # path_movement_group.setStyleSheet("border:1px solid rgb(41, 41, 41); ")
    ...
    def _set_path_raw_length(self, value: float):
        self._path_raw_length.setText(f"{value:>12.4f}")

    def _set_path_length(self, value: float):
        self._path_length.setText(f"{value:>12.4f}")

    def _set_start_pt(self, start_x: float, start_y: float, start_z: float):
        self._start_x.setText(f"{start_x:>12.4f}")
        self._start_y.setText(f"{start_y:>12.4f}")
        self._start_z.setText(f"{start_z:>12.4f}")

    def _set_end_pt(self, end_x: float, end_y: float, end_z: float):
        self._end_x.setText(f"{end_x:>12.4f}")
        self._end_y.setText(f"{end_y:>12.4f}")
        self._end_z.setText(f"{end_z:>12.4f}")
